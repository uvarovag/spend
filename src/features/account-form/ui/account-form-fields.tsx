...
// The actual body of the account create/edit modal (`pages/accounts/ui/account-form-screen.tsx`) —
// pulled out so it can also be embedded directly as an onboarding step, instead of that screen
// being re-navigated to or reimplemented. Only the surrounding chrome (ModalHeader, SafeAreaView,
// what "done" means — `router.back()` for the modal, advancing a step for onboarding) stays with
// each caller.
export function AccountFormFields({ existingAccount, locale, onSaved, onArchived }: AccountFormFieldsProps) {
