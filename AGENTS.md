...
- Default design lens for new code: **DRY**, **KISS**, **YAGNI**, low coupling, high cohesion. DRY — reuse an existing shared function/hook/constant instead of copying logic across files (e.g. `accountFrequencyWindowInDays` in `entities/transaction/model/account-role.ts` is defined once and imported everywhere that needs the same window, not re-declared per call site). KISS — prefer the simplest structure that actually solves the problem, not the most general one. YAGNI — same spirit as the lean-code rule above, don't design for hypothetical future requirements. Low coupling / high cohesion — keep each module's public surface small and its internals about one concern; `store-bridge.ts`/`db-bridge.ts` exist specifically to decouple entity files from `store.ts`/`db.ts` internals (see the two sections above) — follow that same instinct for new cross-module dependencies.
